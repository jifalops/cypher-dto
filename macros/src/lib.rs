...
#[proc_macro_derive(CypherNode, attributes(name, id))]
...
#[proc_macro_derive(CypherRelation, attributes(name, id))]
...
///
/// On structs marked with `#[derive(CypherNode)]` or `#[derive(CypherRelation)]`,
...
/// Shorthand for `#[derive(CypherNode)]`, `#[stamps]`, and other common derive implementations.
///
...
/// Shorthand for `#[derive(CypherRelation)]`, `#[stamps]`, and other common derive implementations.
///
/// The default derives are `Clone`, `Debug`, `PartialEq`, and if the `serde` feature is enabled: `Serialize`, and `Deserialize`.
#[proc_macro_attribute]
